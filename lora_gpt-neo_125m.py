"""
замеры GPT-Neo-125m + LoRA на едином бенчмарке
"""
...
dataset_path = "data_results/step1_res/GPT_Neo_125m_results_4k.csv"
...
def loading_model():
    model_path = "./model_gpt-neo_125m"  # локальный путь
...
models_list = ["GPT_Neo_125m"]

attempt_list = [1]

model_path = "./model_gpt-neo_125m"
adapters = [
    "lora_models/GPT_Neo_125m_att", 
    "lora_models/GPT_Neo_125m_attmlp", 
    "lora_models/GPT_Neo_125m_mlp"
]

import warnings
warnings.filterwarnings('ignore')

save_path = "data_results/step1_res/GPT_Neo_125m_results_4k.csv"
...
for m in range(len(models_list)):
    for i in attempt_list:
        for adapter in adapters:
            ...
            if adapter == "lora_models/GPT_Neo_125m_att":
                answer_col = f"{models_list[m]}_att_{i}"
            elif adapter == "lora_models/GPT_Neo_125m_attmlp":
                answer_col = f"{models_list[m]}_attmlp_{i}"
            elif adapter == "lora_models/GPT_Neo_125m_mlp":
